...
public class GameLuckyConfigServiceImpl extends ServiceImpl<GameLuckyConfigMapper, GameLuckyConfig> implements GameLuckyConfigService {
    ...
    public boolean addLuckyConfig(List<GameLuckyConfig> configList) {
        ...
        int roundId = 1;
        GameLuckyRound roundCount = gameLuckyRoundService.getOne(new QueryWrapper<GameLuckyRound>().orderByDesc("round_count").last("limit 1"));
        if(roundCount == null){
            roundId =1;
    ...
    public List<GameLuckyConfig> getNewestLuckyConfig(Integer luckyId) {
        // 获取最后一次的 round_id
        GameLuckyRound gameLuckyRound = gameLuckyRoundService.getOne(new QueryWrapper<GameLuckyRound>().orderByDesc("id").last("limit 1"));
