...
public class GameMyCardServiceImpl extends ServiceImpl<GameMyCardMapper, GameMyCard> implements GameMyCardService {
     ...
     * @return
     */
    public GameCost getSkillTotal(Integer userId) {
        List<GameCost> list = baseMapper.getSkillTotal(userId);
        GameCost gameCost = new GameCost();
        BigDecimal incCoin = BigDecimal.ZERO;
        BigDecimal incExperience = BigDecimal.ZERO;
        BigDecimal lowPercent = BigDecimal.ZERO;
        BigDecimal topPercent = BigDecimal.ZERO;
        for (int i = 0; i < list.size(); i++) {
            GameCost tempGameCost = list.get(i);
            incCoin = incCoin.add(tempGameCost.getIncCoin());
            incExperience=incExperience.add(tempGameCost.getIncExperience());
            lowPercent=lowPercent.add(tempGameCost.getLowPercent());
            topPercent= topPercent.add(tempGameCost.getTopPercent());
        }
        gameCost.setIncCoin(incCoin);
        gameCost.setIncExperience(incExperience);
        gameCost.setLowPercent(lowPercent);
        gameCost.setTopPercent(topPercent);
        return gameCost;
    }
